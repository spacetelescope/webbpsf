...
class WebbPSF_GUI(wx.Frame):
    ...
    def _add_labeled_dropdown(self, name, parent, parentsizer, label="Entry:", choices=None, default=0, width=None, position=(0, 0),
                              columnspan=1, expand=True, **kwargs):
    ...
    def _create_widgets_wx(self):
        ...
        for iname in insts:
            ...
            panelSizer.Add(wx.StaticText(inst_panel, label='Configuration Options for ' + iname + "     "), (0, 0), (1, 3),
                           flag=wx.ALIGN_LEFT)
            ...
            self._add_labeled_dropdown(iname + "_filter", inst_panel, panelSizer, label='    Filter:',
                                       choices=self.instrument[iname].filter_list,
            ...
            if len(self.instrument[iname].image_mask_list) > 0:
                ...
                masks.insert(0, "")
                self._add_labeled_dropdown(iname + "_pupil", inst_panel, panelSizer, label='    Pupil Stop:', choices=masks,
                                           position=(3, 0))

                fr2 = wx.Panel(inst_panel)
                fr2Sizer = wx.GridBagSizer()
                self._add_labeled_entry(iname + "_pupilshift_x", fr2, fr2Sizer, label='  pupil shift in X:', value='0', width=40,
                                        position=(0, 4))
            ...
            panelSizer.Add(wx.StaticText(inst_panel, label='Configuration Options for the Telescope (OTE) '), (5, 0), (1, 5),
                           flag=wx.ALIGN_LEFT)
            ...
            self._add_labeled_dropdown(iname + "_opd", opdPanel, opdSizer, label='    OPD File:', choices=opd_list, default=default_opd,
                                       width=220, position=(0, 0), expand=False)
            self.Bind(wx.EVT_COMBOBOX, self.ev_update_OPD_labels, self.widgets[iname + "_opd"])

            self._add_labeled_dropdown(iname + "_opd_i", opdPanel, opdSizer, label=' # ', choices=[str(i) for i in range(10)], width=3,
                                       position=(0, 2), expand=False)
        ...
        r = 0
        self._add_labeled_entry('FOV', calcPanel, calcSizer, label='Field of View:', value=str(conf.default_fov_arcsec),
                                postlabel='arcsec/side', position=(0, 0))
        r += 1
        self._add_labeled_entry('detector_oversampling', calcPanel, calcSizer, label='Output Oversampling:', width=3,
                                value=str(conf.default_oversampling), postlabel='x finer than instrument pixels       ', position=(r, 0))

        r += 1
        self._add_labeled_entry('fft_oversampling', calcPanel, calcSizer, label='Coronagraph FFT Oversampling:', width=3,
                                value=str(conf.default_oversampling), postlabel='x finer than Nyquist', position=(r, 0))
        r += 1
        self._add_labeled_entry('nlambda', calcPanel, calcSizer, label='# of wavelengths:', width=3, value='', position=(r, 0),
                                postlabel='Leave blank for autoselect')
        r += 1

        self._add_labeled_dropdown("jitter", calcPanel, calcSizer, label='Jitter model:',
                                   choices=['Just use OPDs', 'Gaussian jitter with 7 mas rms', 'Gaussian jitter with 30 mas rms'], width=20,
                                   position=(r, 0), columnspan=2)
        r += 1
        output_options = ['Oversampled PSF only', 'Oversampled + Detector Res. PSFs', 'Mock full image from JWST DMS']
        self._add_labeled_dropdown("output_format", calcPanel, calcSizer, label='Output Format:',
                                   choices=['Oversampled image', 'Detector sampled image', 'Both as FITS extensions',
                                            'Mock JWST DMS Output'], width=30, position=(r, 0), columnspan=2)
    ...
    def ev_displayPSF(self, event):
        ...
        poppy.display_PSF(self.PSF_HDUlist, vmin=self.advanced_options['psf_vmin'], vmax=self.advanced_options['psf_vmax'],
                          scale=self.advanced_options['psf_scale'], cmap=self.advanced_options['psf_cmap'],
    ...
    def ev_displayOPD(self, event):
        ...
        if self.inst.pupilopd is None:
            tkMessageBox.showwarning(
                message="You currently have selected no OPD file (i.e. perfect telescope) so there's nothing to display.",
                title="Can't Display")
        else:
            if self._enable_opdserver and 'ITM' in self.opd_name:
                ...
                opd = fits.getdata(
                    self.inst.pupilopd[0])  # in this case self.inst.pupilopd is a tuple with a string so we have to load it here.
    ...
    def _updateFromGUI(self):
        ...
        if self.advanced_options['monochromatic']:
            # monochromatic mode
            self.nlambda = 1
            self.monochromatic_wavelength = float(
                self.widgets['nlambda'].GetValue()) * 1e-6  # note that the GUI is in microns, so convert to meters here.
...
class WebbPSFPreferencesDialog(WebbPSFDialog):
    ...
    def _createWidgets(self):
        ...
        r += 1
        self._add_labeled_dropdown("use_multiprocessing", panel3, sizer, label='    Use Multiprocessing for DFTs:',
                                   choices=['True', 'False'],
