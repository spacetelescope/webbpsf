...
def _read_opd(filename):
    ...
    full_file_path = os.path.join(
        webbpsf.utils.get_webbpsf_data_path(), "MAST_JWST_WSS_OPDs", filename
    )
...
def get_datetime_utc(opdhdul, return_as="string"):
    ...
    if return_as == "string":
        return (
            opdhdul[0].header["DATE-OBS"] + " " + opdhdul[0].header["TIME-OBS"][:-4]
        )  # leave off milliseconds
    elif return_as == "astropy":
        return astropy.time.Time(
            opdhdul[0].header["DATE-OBS"] + "T" + opdhdul[0].header["TIME-OBS"],
            format="isot",
            scale="utc",
        )
    else:
...
def wavefront_time_series_plot(
    ...
    is_routine = np.asarray(
        [int(v[1:6]) in routine_pids for v in opdtable[where_pre]["visitId"]]
    )
    ...
    for row, rms in zip(opdtable[where_post], rms_nm[where_post]):
        ...
        if sum(w) > 0:
            ...
            plt.plot(
                [d_pre.plot_date, d_post.plot_date], [r_pre, r_post], color="green"
            )

    # Label visit IDs for routine WFS
    if label_visits:
        for row, rms in zip(
            opdtable[where_pre][is_routine], rms_nm[where_pre][is_routine]
        ):
            d_pre = astropy.time.Time(row["date"], format="isot")
            visit = row["visitId"]
            short_visit = f"  {int(visit[1:6])}:{int(visit[6:9])}"
            if (
                d_pre.datetime >= start_date
                and d_pre.datetime <= end_date
                and rms < ymax
            ):
                plt.text(
                    d_pre.plot_date,
    ...
    plt.plot(
        dates.plot_date[where_pre][is_routine],
        rms_nm[where_pre][is_routine],
        ls="none",
        alpha=0.5,
    )
    plt.plot(
        dates.plot_date[where_post],
        rms_nm[where_post],
        ls="none",
        color="C2",
        alpha=0.5,
    ...
    plt.title(
        "Observatory WFE over time at NIRCam Field Point 1",
        fontweight="bold",
        fontsize=15,
    )
...
def wfe_histogram_plot(
    ...
    opdtable1 = webbpsf.mast_wss.filter_opd_table(
        opdtable0, start_time=start_date, end_time=end_date
    )
    ...
    axes[0].set_title(
        f"{fig_title} WFE from {start_date.isot[0:10]} to {end_date.isot[0:10]}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    if thresh:
        axes[0].axhline(
            thresh, color="C2", label="OTE Correction threshold", linestyle="dashed"
        )
    ...
    axes[1].set_ylabel(
        "Fraction of time with this WFE", fontweight="bold", color="#1f77b4"
    )
    ...
    if pid:
        for i, obs_date in enumerate(pid_dates):
            axes[0].axvline(
                obs_date.plot_date, color="darkgrey", ls="--", alpha=0.5, zorder=1
            )
...
def show_opd_image(
    ...
    if ax is None:
        ...
        im = plt.imshow(
            array, cmap=matplotlib.cm.RdBu_r, vmin=-vmax, vmax=vmax, origin="lower"
        )
        ax = im.axes
    else:
        im = ax.imshow(
...
def _get_mimf2_focus_offset_model(npix=256):
    ...
    ote.move_sm_local(
        piston=-1.789125, trans_unit="micron"
    )  # Piston from M2022042201_sur.xml
...
def single_measurement_trending_plot(
    ...
    if subtract_target:
        ...
        target_1024 = astropy.io.fits.getdata(was_targ_file)
        target_256 = (
            poppy.utils.krebin(target_1024, (256, 256)) / 16
        )  # scale factor for rebinning w/out increasing values

        opd -= target_256
        prev_opd -= target_256

    # Compute deltas and decompose
    deltatime = get_datetime_utc(opdhdu, return_as="astropy") - get_datetime_utc(
        prev_opd_hdu, return_as="astropy"
    )
    ...
    if reference is None:
        ...
        actual_ref = min(
            astropy.time.Time(opdtable["date"]),
            key=lambda x: abs(x - astropy.time.Time(reference)),
        ).value
    ...
    if opdtable[row_index]["is_pre_correction"]:
        ...
        match = opdtable[
            (opdtable["visitId"] == opdtable[row_index]["visitId"])
            & opdtable["is_post_correction"]
        ][0]
        ...
        if verbose:
            print(
                "   That measurement has a correction, therefore reading in post-move sensing data and SUR."
            )
    ...
    show_opd_image(
        ...
        vmax=vmax,
        mask=mask,
        maskc3=mask_without_C3,
        fontsize=fontsize,
    )
    iax.set_title(
        f"{meas_title}\n{get_datetime_utc(opdhdu)}",
    ...
    if show_post_move:
        ...
        fig.text(
            0.55,
            0.77,
            "Sensing-only visit. No mirror moves.",
            alpha=0.3,
            horizontalalignment="center",
            fontsize=fontsize,
        )
    ...
    iax = axes[1, 1]
    show_opd_image(
        delta_opd, ax=iax, vmax=vmax, deltatime_hrs=deltatime_hrs, fontsize=fontsize
    )
    iax.set_title(
    ...
    iax = axes[1, 2]
    show_opd_image(
        fit, ax=iax, vmax=vmax, deltatime_hrs=deltatime_hrs, fontsize=fontsize
    )
    ...
    iax = axes[2, 1]
    show_opd_image(
        delta_opd2 * nanmask, ax=iax, vmax=vmax, mask=mask, fontsize=fontsize
    )
    ...
    if show_correction:
        ...
        show_opd_image(
            -correction, ax=iax, vmax=vmax, mask=correction_mask, fontsize=fontsize
        )
        iax.set_title(
    ...
    iax = axes[2, 3]
    show_opd_image(
        delta_opd2 - fit2, ax=iax, vmax=vmax, maskc3=mask_without_C3, fontsize=fontsize
    )
    iax.set_title("High order WFE\nin difference", fontsize=fontsize * 1.1)
    ...
    mappable_nm = matplotlib.cm.ScalarMappable(
        cmap=cmap, norm=matplotlib.colors.Normalize(-vmax * 1000, vmax * 1000)
    )
    ...
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list(
        "thresholds", ["LightGreen", "Yellow", "Salmon"], N=3
    )
    ...
    cax.plot(
        np.arange(18) / (64) + 0.02, ptts[:, 0], marker="+", ls="none", color="black"
    )
    cax.plot(
        np.arange(18) / (64) + 0.34, ptts[:, 1], marker="+", ls="none", color="navy"
    )
    cax.plot(
        np.arange(18) / (64) + 0.67, ptts[:, 2], marker="+", ls="none", color="black"
    )
    cax.set_ylim(0, 0.15)
    fig.text(
        0.89,
...
def series_of_measurement_trending_plots(
    ...
    with PdfPages(filename) as pdf:
        for row_index in reversed(range(len(opdtable))):
            ...
            single_measurement_trending_plot(
                opdtable, row_index=row_index, ignore_missing=ignore_missing
            )
...
def cleanup_global_tt_for_wing_tilt(delta_opd, wing="left"):
    ...
    if wing == "left":  # -V2 wing
        bigtiltmask = (
            (segmask != 0) & (segmask != 17) & (segmask != 16) & (segmask != 15)
        )
    elif wing == "right":  # +V2 wing
        bigtiltmask = (
            (segmask != 0) & (segmask != 9) & (segmask != 10) & (segmask != 11)
        )
    else:
        raise ValueError(
            "Invalid value for wing parameter. Must be 'left' or 'right'. "
        )
...
def wavefront_drift_plots(
    ...
    mappable_nm = matplotlib.cm.ScalarMappable(
        cmap=matplotlib.cm.RdBu_r, norm=matplotlib.colors.Normalize(-vmax, vmax)
    )
    fig.colorbar(mappable_nm, cax=cax, label="WFE [nm]")
...
def check_colnames(opdtable):
    ...
    if all(colname in colnames for colname in opdtable.colnames) is False:
        ...
        if all(colname in opdtable.colnames for colname in colnames) is False:
            raise KeyError(
                "Expected the following column names in the opdtable: " + str(colnames)
            )
        else:
...
def get_dates_for_pid(pid, project="jwst"):
    ...
    try:
        ...
        obs_table["obs_num"] = [
            x["obs_id"][7:10] if x["calib_level"] == 2 else x["obs_id"][8:13]
            for x in obs_table
        ]
...
def monthly_trending_plot(
    ...
    for row_index in range(len(opdtable)):
        ...
        wfe_ote = (
            inst.pupilopd["PRIMARY"].data + inst.pupilopd["SENSING_OTE_FD_WFE"].data
        )

        cid = opdtable[row_index]["fileName"][0:9]
        if cid in cids_with_left_wing_tilts:
            vprint(
                "  Adjusting delta OPD to exclude left wing (-V2) from global PTT, for wing tilt"
            )
            wfe_ote = cleanup_global_tt_for_wing_tilt(wfe_ote)
        elif cid in cids_with_right_wing_tilts:
            vprint(
                "  Adjusting delta OPD to exclude right wing (+V2) from global PTT, for wing tilt"
            )
    ...
    sensing_tds = astropy.time.Time(
        corrections_table["Post Move Sensing Time"]
    ) - astropy.time.Time(corrections_table["Pre Move Sensing Time"])
    correction_times = (
        astropy.time.Time(corrections_table["Pre Move Sensing Time"]) + sensing_tds / 2
    )
    ...
    for sf in subfigs:
        sf.set_facecolor(
            "none"
        )  # work around display issue that was hiding the connection patches
    ...
    axes = subfigs[1].subplots(
        2, 1, gridspec_kw={"left": 0.07, "right": 0.98, "top": 0.97}
    )

    fs = 14  # Font size for axes labels

    fig.suptitle(
        f"WF Trending for {year}-{month:02d}", fontsize=fs * 1.5, fontweight="bold"
    )
    ...
    for ax in axes[0:2]:
        ...
        ax.xaxis.set_major_locator(
            matplotlib.dates.WeekdayLocator(
                byweekday=matplotlib.dates.MONDAY, interval=1
            )
        )
    ...
    for i, row in enumerate(opdtable):
        ...
        if row["wfs_measurement_type"] == "pre":
            ...
            rms_label = im_axes[0, plot_index].text(
                20,
                20,
                f"{webbpsf.utils.rms(wfes_ote[i], mask=apmask)*1e9:.1f}",
                color="yellow",
                fontsize=fs * 0.6,
            )
            ...
            im_axes[1, plot_index].text(
                20,
                20,
                f"{webbpsf.utils.rms(delta_opd, mask=apmask)*1e9:.1f}",
                color="yellow",
                fontsize=fs * 0.6,
            )
            ...
            if rms_label is not None:
                ...
                del (
                    rms_label
                )  # delete the previously-written one for the pre-move sensing
...
def plot_phase_retrieval_crosscheck(fn, vmax_fraction=1.0):
    ...
    norm = matplotlib.colors.LogNorm(
        wlm8_m.max() / 1e4 * vmax_fraction, wlm8_m.max() * vmax_fraction
    )
    ...
    fig.suptitle(
        f"{hdul[0].header['CORR_ID']}     {hdul[0].header['TSTAMP']}     {fn}",
        fontsize=18,
        fontweight="bold",
    )
    ...
    plt.colorbar(
        mappable=axes[0, 0].images[0], cax=cax, label="Surface Brightness [MJy/sr]"
    )
...
def plot_wfs_obs_delta(fn1, fn2, vmax_fraction=1.0):
    ...
    norm = matplotlib.colors.LogNorm(
        wlm8_m1.max() / 1e4 * vmax_fraction, wlm8_m1.max() * vmax_fraction
    )
...
def show_wfs_around_obs(filename, verbose="True"):
    ...
    get_datetime = lambda header: astropy.time.Time(
    ...
    gs = matplotlib.gridspec.GridSpec(
        2, 4, figure=fig, hspace=0.3, height_ratios=[1, 2]
    )
    ...
    ax_t.plot_date(
        [wfe_before_dateobs.plot_date, dateobs.plot_date, wfe_after_dateobs.plot_date],
        [0, 0, 0],
    )
    ax_t.axhline(0, ls=":")
    ...
    ax_t.set_title(
        f"Wavefront Sensing around Observation\n {fnbase}",
        fontweight="bold",
        fontsize=18,
    )
    ...
    # Plot the OPDs
    vmax = 0.15
    webbpsf.trending.show_opd_image(
        wfe_before * nanmask * 1e6, ax=ax1, vmax=vmax, fontsize=10
    )
    ax1.set_title("WFS Before\n ", color="C0", fontweight="bold")
    webbpsf.trending.show_opd_image(
        wfe_weighted * nanmask * 1e6, ax=ax2, vmax=vmax, fontsize=10
    )
    ax2.set_title(
        "Time-weighted Linear \nEstimate at Obs Time", color="C2", fontweight="bold"
    )
    webbpsf.trending.show_opd_image(
        wfe_after * nanmask * 1e6, ax=ax3, vmax=vmax, fontsize=10
    )
    ax3.set_title("WFS After\n ", color="C0", fontweight="bold")

    webbpsf.trending.show_opd_image(
        (wfe_after - wfe_before) * nanmask * 1e6, ax=ax4, vmax=vmax, fontsize=10
    )
...
def show_wfs_ta_img(visitid, ax=None, return_handles=False):
    ...
    ax.text(
        0.05, 0.9, hdul[0].header["TARGPROP"], color="white", transform=ax.transAxes
    )
...
def nrc_ta_image_comparison(visitid):
    ...
    hdul, ax, norm, cmap, bglevel = show_wfs_ta_img(
        visitid, ax=axes[0], return_handles=True
    )
    ...
    dofs = (
        np.isfinite(diffim).sum() - 4
    )  # 4 estimated parameters: X and Y offsets, flux scaling, background level
