...
class OTE_Linear_Model_Elliott(OPD):
    ...
    def __init__(self, opd=None,  opd_index=0, transmission=None, rm_ptt=False, zero=False):
        ...
        OPD.__init__(self, opd=opd,  opd_index=slice, transmission=transmission)
    ...
    def _apply_zernikes_to_seg(self, segment, zernike_coeffs, coordsys='local', debug=False):
        ...
        if 'B' in segment: ang += 180 # As are opposite Bs
        elif 'C' in segment: ang += 60 # Cs are 60 deg back
        ...
        Rw = np.sqrt(Xrc_elliott**2 + Yrc_elliott**2)/seg_radius  # This normalizes the Zernike input radial coord to maximum 1 on the segment aperture.
    ...
    def tilt(self, segment, tiltX=0.0, tiltY=0.0, tiltZ=0.0, unit='urad', display=False, coordsys='elliott'):
        ...
        elif unit == 'milliradian' or unit=='mrad': tilts*= 1e3
        else: raise NotImplementedError('unknown unit')
    ...
    def sinewave(self, cyclesX = 0.0, cyclesY = 0.0, amplitude = 100.,  display=True):
        ...
        self.opd[wsegs] += np.cos(   2*np.pi*( X*cyclesX + Y*cyclesY ) /(ref_ap_diam/self.pixelscale) )[wsegs] * (amplitude/1000.)  # convert amplitude to microns
...
class OTE_Linear_Model_WSS(OPD):
    ...
    def move_sm_local(self, xtilt=0.0, ytilt=0.0,  rot_unit='urad',
        ...
        if np.abs(vector).sum() > 0:
            ...
            trans_unit = trans_unit.lower()
            if trans_unit == 'micron' or trans_unit =='um': pass
            elif trans_unit =='millimeters' or trans_unit =='millimeter' or trans_unit =='mm': vector *= 1000
            elif trans_unit =='nm' or trans_unit =='nanometer' or trans_unit =='nanometers' : vector /= 1000
            else: raise ValueError("Unknown trans_unit for length: %s" % trans_unit)
...
##### Deprecated code - now merged into "move_seg_local" #####
#     def tilt(self, segment, xtilt=0.0, ytilt=0.0, clocking=0.0, unit='urad', display=False):
#         """ Tilt/rotate a segment some angle around X, Y, or Z (clocking).
#
#         These motions are always commanded in the segment-local "Control" coordinate systems,
#         which are distinct for each segment.
#
...
#         self.update_opd(display=display)
#
#     def roc(self,segment, roc=0.0, unit='micron', display=False):
#         """ Apply radius of curvature command
#         """
#         if unit != 'micron': raise NotImplementedError('no units conv for ROC yet')
#
#         iseg = np.where(self.segnames ==segment)[0][0]
#         self.segment_state[iseg][5] += roc
#         self.update_opd(display=display)
#
#     def displace(self, segment, radial=0.0, piston=0.0, unit='micron', display=False):
#         """ Move a segment some distance in radial decenter or piston
#
#         Note that this linear model doesn't support azimuthal decenter, and segments
#         are not expected to be active controlled in that degree of freedom in practice.
#         This is OK due to the degeneracy of the control space.
#
#         Parameters
#         -----------
#         segment : str
#             Segment name, e.g. 'A1'
#         radial, piston: floats
#             Displacement distance, in microns by default.
#         unit : str
#             Unit for displacements. Can be 'micron', 'millimeter','nanometer', 'mm', 'nm', 'um'
#         display : bool
#             Display after moving?
#         coordsys : string
#             Name of coordinate system the input tilts are with respect to. Can be 'local', 'global' or 'elliott'
#         """
#
...
class OteThermalModel(object):
    ...
    def __init__(self, delta_time):
        ...
        try:
            self.input_time = delta_time[0] * delta_time[1]
        except AttributeError:
            self.input_time = delta_time[0] * u.minute
